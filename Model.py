...
def encode_faces(training_dir=r'E:\Coding\Vs Code Studio\face_recog_env\data\training'):
    known_encodings = []
    known_names = []
    ...
    user_dirs = [d for d in os.listdir(training_dir) if os.path.isdir(os.path.join(training_dir, d))]
    total_images = sum(len(files) for _, _, files in os.walk(training_dir))
    ...
    with open(r'E:\Coding\Vs Code Studio\face_recog_env\encodings.pkl', 'wb') as f:
